...
const Header = () => {
  ...
  const getCategory = (e, cat) => {
    e.preventDefault()
    navigate(`/pengacara/${cat}`)
  }
  return (
      <div className='flex flex-col justify-center items-center h-auto md:h-[700px] mx-auto my-auto' style={{ background: 'linear-gradient(#7D8797 50%, #ffffff 50%)' }}>
          ...
          <div className='max-w-7xl'>
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mt-8 mb-8 gap-4 md:gap-8'>
                  <CardHeader img={Icons} title='Hukum Bisnis' onClick={(e) => getCategory(e, 'Hukum Bisnis')} />
                  <CardHeader img={Icons} title='Hukum Pidana' onClick={(e) => getCategory(e, 'Hukum Pidana')}/>
                  <CardHeader img={Icons} title='Hukum Kontrak' onClick={(e) => getCategory(e, 'Hukum Kontrak')}/>
                  <CardHeader img={Icons} title='Hukum Properti' onClick={(e) => getCategory(e, 'Hukum Properti')} />
              </div>

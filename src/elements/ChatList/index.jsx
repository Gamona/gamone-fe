...
const ChatList = ({partnerId, senderId}) => {
  ...
  }, [chats, lawyerId, mounted, partnerId, senderId, userId])
  ...
  return (
    ...
    >
      ...
      {chats.length > 0 && chats.map((cur, keys) => {
            return (
              <>
                <div className="flex justify-center mb-2" key={`chat-${keys}`}>
                <div
                  className="rounded py-2 px-4"
                  style={{ backgroundColor: "#DDECF2" }}
                >
                  <p className="text-sm uppercase">{cur.date}</p>
                </div>
                </div>
                {cur.data.map((current, key) => {
                  return (
                    <div className={`flex mb-2 ${current.data.sendBy == userId ? 'justify-end' : ''}`} key={key}>
                    <div
                      className="rounded py-2 px-3"
                      style={current.data.sendBy == userId ? { backgroundColor: "#E2F7CB" } : { backgroundColor: "#F2F2F2" }}
                    >
                      <p className="text-sm text-teal">{current.data.sendBy}</p>
